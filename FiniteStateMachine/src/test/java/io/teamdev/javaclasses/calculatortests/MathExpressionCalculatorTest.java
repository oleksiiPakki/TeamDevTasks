...
class MathExpressionCalculatorTest {
    ...
    @ParameterizedTest
    @CsvSource({"*8/2-11, 0"})
    void testEvaluationOfInCorrectFormatExpression(String input, int expected)
            throws IncorrectFormatOfExpressionException {

        IncorrectFormatOfExpressionException ex = assertThrows(
                IncorrectFormatOfExpressionException.class,
                () -> mathCalculator.evaluate(input));

        assertTrue(ex.getMessage().contains("Incorrect format, deadlock"));
        assertThat(ex.errorPosition()).isEqualTo(expected);
    }
    ...
    @ParameterizedTest
    @CsvSource(value = {"3 > 2 : true", "PI() >= PI() : true", "max(max(4,10)) < min(2+2,3+3) * 2 : false"}, delimiter = ':')
